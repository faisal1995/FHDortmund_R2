...
pollutantmean <- function(directory, pollutant, id = 1:332) {
    ...
    means <- c()
    p <- "C:/Users/Faisal/Desktop/Lecture#1/RAssignment/FHDortmund_R2/"

   for(i in id){
       
         dir <- paste(p, directory, "/",  sprintf("%03d", i), ".csv", sep = "")
        dataRead <- read_csv(dir)
        dataRead2 <- dataRead[pollutant]
        means <- c(means, dataRead2[!is.na(dataRead2)])
    }
...
```{r}
pollutantmean("specdata", "sulfate", 55)
...
complete <- function(directory, id = 1:332) {
    ...
    for(i in id){
         ...
         dataRead <- read.csv(dir)
        dataRead2 <- dataRead[(!is.na(dataRead$sulfate)), ]
        dataRead2 <- dataRead2[(!is.na(dataRead2$nitrate)), ]
        nobs <- nrow(dataRead2)
...
corr <- function(directory, threshold = 0) {
  ...
  p <- "C:/Users/Faisal/Desktop/Lecture#1/RAssignment/FHDortmund_R2/"
     ...
     # I used the previous function "Complete" to obtain the complete cases for this Part
    c_cases <- complete(directory)
    
    #To get the vector where the number of complete cases greater than the threshold
    c_cases <- c_cases[c_cases$nobs>=threshold, ]
    ...
    if(nrow(c_cases)>0){
        for(i in c_cases$id){
            ...
            dataRead2 <- dataRead[(!is.na(dataRead$sulfate)), ]
            dataRead2 <- dataRead2[(!is.na(dataRead2$nitrate)), ]
            sulfate_data <- dataRead2[2]
            nitrate_data <- dataRead2[3]
...
```{r}
c <- corr("specdata", 5000)

head(c); summary(c) ; length(c)
